...
For the new one-action bulk workflow, use
`POST /api/admin/invigilator-assignments/auto-assign` with no body, review the
results, then `POST /api/admin/invigilator-assignments/publish` with
`{"examSessionIds": [10, 11]}`. See
[automatic assignment documentation](invigilator-assignment-hierarchy.md).
The following is the optional single-exam workflow; its manual and auto-assign
requests require the academic selection documented at that link.
...
4. Optionally use `POST /api/admin/invigilator-assignments` for a manual draft or `POST /api/admin/invigilator-assignments/{examSessionId}/{venueId}/{staffId}/cancel` to remove an assignment. Cancelling either a draft or a published assignment deletes its database row.
